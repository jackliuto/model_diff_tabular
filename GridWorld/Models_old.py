...
class DPAgent:
    ...
    def value_iteration(self, rank_V = [], init_V = [], T=None, max_steps=np.inf, det_policy = False):
...
class QLearningAgent:
    ...
    def choose_aciton_boltz(self, s, V_heur, temp):
        if V_heur == []:
            a = self.env.action_space.sample()
        else:
            q = []
            for a in range(self.env.nA):
                for prob, next_state, reward, done in self.env.P[s][a]:
                    q.append(V_heur[next_state]+0.0001) #take account zero divison
            softmax_q = self.softmax_t(q,temp=temp)
            # print(softmax_q)
    ...
    def run_eps(self, V_heur, temp, max_step, num_eps, boltz=True):
        # if V_heur == []:
        #     V_heur = np.ones(self.env.nS)
        num_steps = []
        dis_reward = []
        for i in range(num_eps):
            ...
            while not t and counter < max_step:
                if boltz:
                    a = self.choose_aciton_boltz(beg_s, V_heur, temp)
                else:
